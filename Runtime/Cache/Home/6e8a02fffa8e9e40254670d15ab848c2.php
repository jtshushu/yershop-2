...
<div class="center_right" id="MemberCenterCtrl" style="display: block;">
	...
	<?php else: ?><form action='<?php echo U("Shopcart/delorder");?>' method="post" name="delform" >
        <div class="order_del">
			  <span>  <input class="checkbox check-all" type="checkbox"> 全选 <a   href='javascript:vod(0)' onclick="delorder()">删除选中</a> </span></div>
        <div class="good-canshu"><span class="good-name">站内信
	  </span></div>


          <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div ><span><input class="ids row-selected" type="checkbox" name="id[]" value="<?php echo ($vo["id"]); ?>"></span>&nbsp;<span><a href="<?php echo U('UserEnvelope/detail?id='.$vo['id']);?>" > <?php echo ($vo["title"]); ?></a></div>
         <!-- 列表详情区域开始 -->  
		...
		<!-- 左边商品区域开始 -->  
		  ...
		  <p  style="text-align:right">时间：<?php echo (date('Y-m-d H:i:s',$vo["create_time"])); ?></p><?php endforeach; endif; else: echo "" ;endif; ?> 
