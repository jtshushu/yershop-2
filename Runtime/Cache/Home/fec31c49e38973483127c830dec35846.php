...
<div class="center_right" id="MemberCenterCtrl" style="display: block;">
    ...
    <form method="post" class="person-info" name="form" action="<?php echo U('update');?>">
        
		
<ul style="margin-left:40px;">
		  <?php if(is_array($information)): $i = 0; $__LIST__ = $information;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$po): $mod = ($i % 2 );++$i;?><li><P><?php if(($faceid) > "0"): ?><img src="/Public/Home/images/info.jpg" width="70" height="70"/><?php else: ?>
	<img src="<?php echo (get_cover($faceid,'path')); ?>" width="70" height="70" /><?php endif; ?></p><p><a href="<?php echo U('UcenterMember/edit?id='.$po['uid']);?>" class="red">修改图像</p>
		   </a></li>
          <li><span>昵称:</span><input type="text" name="nickname" value="<?php echo ($po["nickname"]); ?>" /> </li>
          <li><span>账号:</span><?php echo get_username();?> </li>
         <li><span>QQ:</span><input type="text" name="qq" value="<?php echo ($po["qq"]); ?>" /> </li>
          <li><span>性别：</span><?php if($po["sex"]=="1"){echo "男";}; if($po["sex"]=="2"){echo "女";}; if($po["sex"]==0){echo "<select name='sex'><option  value='1'>男</option><option name='sex' value='2'>女</option></select>";}; ?>
		  
		  </li>
           <li><span>生日：</span><input type="text" name="birthday" value="<?php echo ($po["birthday"]); ?>" /> </li>
            
             <li><input type="hidden" name="uid" value="<?php echo ($po["uid"]); ?>" /> </li><BR>
             <li><input type="submit" class="btn-xiugai orange"  value="修改" /> </li><BR><?php endforeach; endif; else: echo "" ;endif; ?>
</ul> 
        </form><?php endif; ?>
