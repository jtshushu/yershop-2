...
<div class="center_right" id="MemberCenterCtrl" style="display: block;">
    ...
    <form method="post" class="person-add" name="form" action="<?php echo U('update');?>">
        
		
<ul style="margin-left:40px;" class="list">
		  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li><span><?php $s=$vo['status'];if($s==1){ echo '<input type="radio" checked="checked" name="id" value="'.$vo['id'].'">'; } if($s==0){ echo '<input type="radio"  name="id" value="'.$vo['id'].'">'; } ?>
		  
		  
		  </span><?php echo ($vo["province"]); ?> <?php echo ($vo["city"]); ?> <?php echo ($vo["area"]); echo ($vo["address"]); ?> <?php echo ($vo["realname"]); ?>  <?php echo ($vo["cellphone"]); ?></li><?php endforeach; endif; else: echo "" ;endif; ?>
<li class="add" style="display:none"><span></span>联系地址<input type="text"  id="address"  name="address">联系人：<input type="text" id="realname" >联系方式<input type="text" id="phone"  ><input type="button" class=" rosy" id="btn-save" value="保存" /></li>
             <li><input type="button" class="btn-xiugai redd"  value="设置默认" onclick="shezhi();"/>&nbsp;<input type="button" class="btn-xiugai blue"  value="删除" onclick="shanchu();"/>&nbsp;<a href='<?php echo U("Address/add");?>' class="btn-xiugai orange"  />增加 </a> </li><BR></ul> 
        </form>
...
    
	$.ajax({
type:'post', //传送的方式,get/post
url:'<?php echo U("deleteAddress");?>', //发送数据的地址
data:{id:val},
 dataType: "json",
success:function(data)
