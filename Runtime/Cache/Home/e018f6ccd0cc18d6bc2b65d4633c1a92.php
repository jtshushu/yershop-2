...
<div class="center_right" id="MemberCenterCtrl" style="display: block;">
	...
	<?php if(empty($list)): ?><div class="personal_info_none_goods">
				<p>没有收藏的物品！<a href='<?php echo U("index/index");?>' class="red">去收藏</a></p>
			
				
			</div>
	<?php else: ?>
       <ul class="shoucang">
 <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$fo): $mod = ($i % 2 );++$i;?><li class="out" onmouseover="this.className='over'" onmouseout="this.className='out'"><h3>收藏时间：<?php echo ($fo["time"]); ?></h3>
		 <p> 
		   <span class="red"> <a href="<?php echo U('Article/detail?id='.$fo['goodid']);?>" > 
			<img src="<?php echo (get_cover(get_cover_id($fo["goodid"]),'path')); ?>"  width="70" height="70"/></a><a href="<?php echo U('Article/detail?id='.$fo['goodid']);?>" ><?php echo (get_good_name($fo["goodid"])); ?> </a> 
			</span>
			
			
			<span class="red">￥<?php echo (get_good_price($fo["goodid"])); ?></span></p>
			 

</li><?php endforeach; endif; else: echo "" ;endif; ?></ul><?php endif; ?>
